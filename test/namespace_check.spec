...
do_check_process_namespace() {
  ...
  namespace_entries_list=$(cat \
    <(for ns in $(ls -1 /proc/$pid/ns/ | grep -v user) ; do readlink /proc/$pid/ns/$ns ; done) \
    <(for ns in $(ls -1 /proc/$check_pid/ns/ | grep -v user) ; do readlink /proc/$check_pid/ns/$ns ; done) \
